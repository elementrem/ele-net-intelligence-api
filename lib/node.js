...
var WS_SECRET = process.env.WS_SECRET || "ele-net-stats-has-a-secret";
...
Node.prototype.getInfo = function()
{
	...
	try {
		this.info.coinbase = web3.ele.coinbase;
		this.info.node = web3.version.node;
		this.info.net = web3.version.network;
		this.info.protocol = web3.toDecimal(web3.version.elementrem);
...
Node.prototype.getLatestBlock = function ()
{
	...
	if(this._web3)
	{
		...
		web3.ele.getBlock('latest', false, function(error, result) {
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			{
				web3.ele.getMining(callback);
			},
			hashrate: function (callback)
			{
				web3.ele.getHashrate(callback);
			},
			gasPrice: function (callback)
			{
				web3.ele.getGasPrice(callback);
			},
			syncing: function (callback)
			{
				web3.ele.getSyncing(callback);
			}
		},
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		web3.ele.getBlockTransactionCount('pending', function (err, pending)
		{
			if (err) {
...
Node.prototype.getHistory = function (range)
{
	...
	{
		web3.ele.getBlock(number, false, callback);
	},
	function (err, results)
	{
		if (err) {
...
Node.prototype.setFilters = function()
{
	...
	this._latestQueue = async.queue(function (hash, callback)
	{
		...
		web3.ele.getBlock(hash, false, function (error, result)
		{
			self.validateLatestBlock(error, result, timeString);
	...
	try {
		this.chainFilter = web3.ele.filter('latest');
	...
	try {
		this.pendingFilter = web3.ele.filter('pending');
